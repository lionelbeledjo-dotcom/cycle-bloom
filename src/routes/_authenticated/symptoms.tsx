...
import { Check, ChevronDown, ChevronUp, Calendar, Loader2 } from "lucide-react";
import { toast } from "sonner";
import { useSymptomLogs, useSaveSymptomLog } from "@/lib/cycle-store";
...
function Symptoms() {
  const { data: history = [], isLoading } = useSymptomLogs();
    ...
    <AppShell title="Suivi du jour">
      ...
      <div className="mt-6 flex justify-center">
        <button onClick={handleSave} disabled={save.isPending}
          className="flex items-center gap-2 rounded-full bg-gradient-to-r from-rose-vif to-violet-doux px-8 py-3.5 text-sm font-semibold text-white shadow-bloom transition hover:scale-[1.02] disabled:opacity-60">
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1 mt-1">
                        {e.moods.length > 0 && <Pill text={`${e.moods.length} humeur${e.moods.length > 1 ? "s" : ""}`} color="violet-doux" />}
                        {e.physical.length > 0 && <Pill text={`${e.physical.length} symptôme${e.physical.length > 1 ? "s" : ""}`} color="orange" />}
                      </div>
                    </div>
...
function Pill({ text, color }: { text: string; color: string }) {
  const map: Record<string, string> = {
    "violet-doux": "bg-violet-doux/10 text-violet-doux",
    "orange": "bg-orange-100 text-orange-700",
  };
  return <span className={`rounded-full px-2 py-0.5 text-[10px] font-medium ${map[color]}`}>{text}</span>;
}
